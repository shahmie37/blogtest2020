...
import com.tecforte.blog.service.util.CommonUtil;
...
public class EntryResource {
    ...
    public ResponseEntity<EntryDTO> createEntry(@Valid @RequestBody EntryDTO entryDTO) throws URISyntaxException {
        ...
        Optional<BlogDTO> blogDTOtemp = blogService.findOne(entryDTO.getBlogId());
        if(blogDTOtemp.isPresent()){
            entryDTO.setBlogIsPositive(blogDTOtemp.get().isPositive());
            entryDTO.setBlogName(blogDTOtemp.get().getName());
        }
        System.out.println("Entry Blog name "+entryDTO.getBlogName());
        System.out.println("Entry Blog isPositive "+entryDTO.getBlogIsPositive());
        if (entryDTO.getId() != null) {
            throw new BadRequestAlertException("A new entry cannot already have an ID", ENTITY_NAME, "idexists");
        }else if(commonUtil.validateEntryEmoji(entryDTO)){
            System.out.println("Flag entry emoji true ");
